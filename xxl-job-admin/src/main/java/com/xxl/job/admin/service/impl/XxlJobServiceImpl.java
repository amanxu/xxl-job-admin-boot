...
>>>>>>> feature/1.0.0
    ...
    @Override
    public Map<String, Object> dashboardInfo(Integer userId) {

        int jobInfoCount = xxlJobInfoDao.findJobCountByUser(userId);
        int jobLogCount = xxlJobLogDao.triggerCountByHandleCodeAndUserId(-1, userId);
        int jobLogSuccessCount = xxlJobLogDao.triggerCountByHandleCodeAndUserId(ReturnT.SUCCESS_CODE, userId);

        // executor count
        Set<String> executerAddressSet = new HashSet<String>();
        List<XxlJobGroup> groupList = xxlJobGroupDao.findByUserId(userId);
    ...
    @Override
    public Map<String, Object> chartInfo(Integer userId, Date startDate, Date endDate) {
        // get cache
        StringBuilder cacheKey = new StringBuilder(TRIGGER_CHART_DATA_CACHE);
        cacheKey.append("_").append(startDate.getTime()).append("_").append(endDate.getTime()).append("_").append(userId);
        Map<String, Object> chartInfo = (Map<String, Object>) LocalCacheUtil.get(cacheKey.toString());
        if (chartInfo != null) {
            return chartInfo;
        }
        ...
        List<Map<String, Object>> triggerCountMapAll = xxlJobLogDao.triggerCountByDayAndUser(userId, startDate, endDate);
        ...
        // set cache 60s
        LocalCacheUtil.set(cacheKey.toString(), result, 60 * 1000);

        return result;
    }

    @Override
    public List<XxlJobInfo> findJobsByGroup(Integer groupId) {
        return xxlJobInfoDao.findJobsByGroup(groupId);
    }
}
